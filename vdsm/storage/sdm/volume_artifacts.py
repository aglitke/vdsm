...
class FileVolumeArtifacts(VolumeArtifacts):
    ...
    def _create_metadata_artifact(self, size, vol_format, prealloc, disk_type,
        ...
        meta = volume.VolumeMetadata(
            ...
            desc,
            volume.LEGAL_VOL)
        self._oop.writeLines(self.meta_volatile_path, meta.format())
...
class BlockVolumeArtifacts(VolumeArtifacts):
    ...
    def _create_lv_artifact(self, parent_vol_id, lv_size):
        ...
        lvm.createLV(self.sd_manifest.sdUUID, self.vol_id, lv_size,
                     activate=True, initialTag=TEMP_VOL_LVTAG)
        tags = [blockVolume.TAG_PREFIX_PARENT + parent_vol_id,
                blockVolume.TAG_PREFIX_IMAGE + self.img_id]
        lvm.changeLVTags(self.sd_manifest.sdUUID, self.vol_id, addTags=tags)

    def _adjust_size_for_dev(self, vol_format, size):
        ...
        if vol_format == volume.RAW_FORMAT:
            dev_size = int(self.sd_manifest.getVSize(self.img_id, self.vol_id))
            if dev_size < size:
                raise se.VolumeCreationError(
                    "The LV %s/%s is smaller than expected" %
                    (self.sd_manifest.sdUUID, self.vol_id))
            if dev_size > size:
                return dev_size
        return size
    ...
    def _create_metadata(self, size, vol_format, prealloc, disk_type, desc,
                         parent_vol_id):
        size = self._adjust_size_for_dev(vol_format, size)
